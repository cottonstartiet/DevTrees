...
function remToPx(rem: string): number {
  ...
  if (typeof window === 'undefined' || typeof document === 'undefined') return num * 16
  const root = document.documentElement
  const base = parseFloat(window.getComputedStyle(root).fontSize)
  return num * (Number.isFinite(base) && base > 0 ? base : 16)
}
...
function readSidebarWidthCookie(): number | null {
  ...
  const match = document.cookie
    .split('; ')
    .find((c) => c.startsWith(`${SIDEBAR_WIDTH_COOKIE_NAME}=`))
  if (!match) return null
  const raw = match.slice(SIDEBAR_WIDTH_COOKIE_NAME.length + 1)
...
function SidebarProvider({
  ...
  const minWidthPx = React.useMemo(() => Math.round(remToPx(SIDEBAR_WIDTH)), [])
  const maxWidthPx = React.useMemo(() => Math.round(remToPx(SIDEBAR_WIDTH_MAX)), [])

  const [width, setWidth] = React.useState<number | null>(() => {
    const stored = readSidebarWidthCookie()
    if (stored == null) return null
    const min = Math.round(remToPx(SIDEBAR_WIDTH))
    const max = Math.round(remToPx(SIDEBAR_WIDTH_MAX))
  ...
  const setWidthPx = React.useCallback(
    (px: number) => {
      ...
      const clamped = Math.round(Math.min(Math.max(px, minWidthPx), maxWidthPx))
      setWidth(clamped)
    ...
    <SidebarContext.Provider value={contextValue}>
      <TooltipProvider delayDuration={0}>
        <div
          ...
          style={
            {
              '--sidebar-width': width != null ? `${width}px` : SIDEBAR_WIDTH,
...
function SidebarResizeHandle({ side }: { side: 'left' | 'right' }): React.JSX.Element | null {
  const { setWidthPx, isResizing, setIsResizing, width, minWidthPx, maxWidthPx, isMobile } =
    useSidebar()
  ...
  const beginDrag = (event: React.PointerEvent<HTMLDivElement>): void => {
    ...
    try {
      ...
      // Ignored: some environments (e.g. tests) don't implement pointer capture
    }
    const startWidth = width != null ? width : minWidthPx
    dragRef.current = {
  ...
  const moveDrag = (event: React.PointerEvent<HTMLDivElement>): void => {
    if (!dragRef.current) return
    const delta = (event.clientX - dragRef.current.startX) * (side === 'left' ? 1 : -1)
    setWidthPx(dragRef.current.startWidth + delta)
  }

  const handleDoubleClick = (): void => {
    setWidthPx(minWidthPx)
  }
  ...
  return (
    <div
      ...
      aria-valuenow={width ?? minWidthPx}
...
function SidebarMenuSubButton({
  ...
  return (
    <Comp
      ...
      data-active={isActive}
      className={cn(
        'flex h-7 min-w-0 -translate-x-px items-center gap-2 overflow-hidden rounded-md px-2 text-sidebar-foreground ring-sidebar-ring outline-hidden group-has-data-[sidebar=menu-action]/menu-sub-item:pr-8 hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0 [&>svg]:text-sidebar-accent-foreground',
