...
/**
 * Renders a single Copilot session as an xterm terminal. On mount it replays the backend buffer
 * (authoritative) then applies live output via the provider, deduplicating with sequence numbers
 * so no output is lost or doubled across the snapshot/subscribe boundary. Live chunks are coalesced
 * onto an animation frame so high-volume output never floods the main thread.
 */
export function TerminalView({ session }: TerminalViewProps): React.JSX.Element {
  ...
  React.useEffect(() => {
    ...
    let appliedSeq = 0
    let ready = false
    const queued: { seq: number; data: Uint8Array }[] = []
    ...
    // Coalesce live writes onto a single animation frame so bursts don't block the UI thread.
    ...
    const scheduleWrite = (data: Uint8Array): void => {
      pending.push(data)
      if (rafId === null) rafId = requestAnimationFrame(flushPending)
    }

    const writeChunk = (event: { seq: number; data: Uint8Array }): void => {
    ...
    void snapshot(sessionId).then((snap) => {
      if (disposed) return
      if (snap) {
        if (snap.buffer.length > 0) term.write(snap.buffer)
