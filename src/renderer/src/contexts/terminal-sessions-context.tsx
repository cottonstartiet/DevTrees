...
export function TerminalSessionsProvider({
  ...
  React.useEffect(() => {
    ...
    void Promise.all([
      ...
      .then(async ([stopUpdates, stopInteractions]) => {
        ...
        const interactions = await Promise.all(
          list.map(async (session) => {
            const revision = interactionRevisionRef.current[session.id] ?? 0
            try {
              return {
                ok: true as const,
                sessionId: session.id,
                revision,
                interaction: await window.api.terminalSessions.interaction(session.id)
              }
            } catch {
              return {
                ok: false as const,
        ...
        for (const update of interactions) {
          if (
            update.ok &&
            (interactionRevisionRef.current[update.sessionId] ?? 0) === update.revision
          ) {
            applyInteraction(update)
          }
        }
      ...
      .catch(() => {
        // Live events can still populate the provider if initial hydration fails.
      })
  ...
  }, [applyInteraction, ingest, mergeEntries])
